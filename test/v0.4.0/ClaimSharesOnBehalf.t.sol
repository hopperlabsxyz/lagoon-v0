...
contract TestClaimSharesOnBehalf is BaseTest {
    ...
    function test_claimSharesOnBehalf() public {
        ...
        assertEq(
            ...
            user1Balance,
            "wrong maxDeposit on user 1"
        );
        assertEq(
            vault.maxDeposit(user2.addr),
            user2Balance,
            "wrong maxDeposit on user 2"
        );
        assertEq(
            vault.maxDeposit(user3.addr),
            user3Balance,
            "wrong maxDeposit on user 3"
        );
        assertEq(
            vault.maxDeposit(user4.addr),
            user4Balance,
            "wrong maxDeposit on user 4"
        );
        assertEq(
            vault.maxDeposit(user5.addr),
            user5Balance,
        ...
        uint256 user1Shares = vault.convertToShares(
            user1Balance,
            user1RequestId
        );
        assertEq(
            ...
            "user1 balance is wrong"
        );
        assertEq(user1Shares, user1Balance * 10 ** vault.decimalsOffset());

        uint256 user2Shares = vault.convertToShares(
            user2Balance,
            user2RequestId
        );
        ...
        assertEq(
            ...
            "user2 balance is wrong"
        );
        assertEq(user2Shares, user2Balance * 10 ** vault.decimalsOffset());

        uint256 user3Shares = vault.convertToShares(
            user3Balance,
            user3RequestId
        );
        ...
        assertEq(
            user3Shares,
            vault.balanceOf(user3.addr),
            "user3 balance is wrong"
        );
        assertEq(user3Shares, user3Balance * 10 ** vault.decimalsOffset());

        uint256 user4Shares = vault.convertToShares(
            user4Balance,
            user4RequestId
        );
        ...
        assertEq(
            user4Shares,
            vault.balanceOf(user4.addr),
            "user4 balance is wrong"
        );
        assertEq(user4Shares, user4Balance * 10 ** vault.decimalsOffset());

        uint256 user5Shares = vault.convertToShares(
            user5Balance,
            user5RequestId
        );
