...
import {InitStruct as ProxyInitStruct, ProxyFactory, VaultProxy} from "@src/ProxyFactory.sol";
...
contract Constants is Test {
    ...
    function setUpVault(uint16 _protocolRate, uint16 _managementRate, uint16 _performanceRate) internal {
        ...
        factory.initialize(address(protocolRegistry), WRAPPED_NATIVE_TOKEN, dao.addr, "V0");

        // we add an implementation
        vm.prank(dao.addr);
        protocolRegistry.updateDefaultLogic("V0", implementation);
