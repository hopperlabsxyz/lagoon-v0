    ...
    ERC4626Upgradeable
{
    ...
    function __ERC7540_init(IERC20 underlying, address wrapped_native_token) internal onlyInitializing {
        ...
        returns (uint256 _depositId)
    {
        ...
        if (msg.value != 0) {
            ...
            if (asset() == address($.wrapped_native_token)) {
                ...
                IWETH9($.wrapped_native_token).transfer(address($.pendingSilo), msg.value);
            } else {
    ...
    function _redeem(uint256 shares, address receiver, address controller)
        internal
        onlyOperator(controller)
    ...
    function _withdraw(uint256 assets, address receiver, address controller)
        internal
        onlyOperator(controller)
    ...
    function _convertToShares(uint256 assets, uint256 requestId, Math.Rounding rounding)
        internal
        view
        returns (uint256)
    {
        ERC7540Storage storage $ = _getERC7540Storage();
        if (requestId == $.redeemId || requestId == $.depositId || requestId == 0) return 0;

        uint256 _totalAssets = $.epochs[requestId].totalAssets + 1;
        uint256 _totalSupply = $.epochs[requestId].totalSupply + 10 ** _decimalsOffset();
        ...
        returns (uint256)
    {
        ...
        if (requestId == $.depositId || requestId == $.redeemId || requestId == 0) return 0;
